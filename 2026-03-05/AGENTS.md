...
在龙虾小队相关频道/子区工作时，**先读 `memory/lobster-squad.md`**。
- 所有任务必须通过钱钱(PM)分配，不能直接给开发
- 通过 Hub API 通信，不是 Discord 消息
- 只有老板(云寒)确认才算完成
- 检验结果立刻写项目文件
...
6. **龙虾小队任务必须通过钱钱（PM）分配，绝不越级直接给小龙/牛牛/花花派 task**
   - 多多 → 钱钱 → 具体 Agent，结果原路回流
   - 越级会导致 result 回给多多而非钱钱，PM 脱离 loop，流程断裂
7. **龙虾小队任务完成后必须立刻通知云寒**
   - 收到 Agent handoff/result 完成通知 → 立刻用 message 发消息给云寒
   - 深夜完成（23:00-08:00）→ 记录待报，下次心跳第一时间汇报
   - 白天完成 → 立刻发，不等不拖
   - **绝不能等云寒来问才说**
8. **前端功能最终测试由多多亲自做**
   - 只有多多有浏览器，其他 agent 都没有（牛牛在东京服务器，小龙/钱钱/花花在 Docker 容器）
   - 牛牛的"测试"只是静态代码分析，不是真正的浏览器测试
   - 任何前端交付前，多多必须：打开页面 → 检查控制台无报错 → 验证功能 → 截图
   - **没有浏览器验证的前端功能不算测试通过**
...
## 反幻觉规则（2026-03-04 云寒指令）

### 操作前必读
- **龙虾小队操作** → 先读 `lobster-squad/STATUS.md`
- **发日报** → 先读 `MEMORY.md` 确认格式和频道
- **部署/重启** → 先读 STATUS.md 确认正确的命令和服务名
- **发消息到 Discord** → 确认目标频道 ID

### 不确定就查，不要猜
- 不记得频道 ID → 读 memory 或 channel-list
- 不记得服务名 → docker compose ps 确认
- 不记得密码/token → 读 memory/credentials.md

### 变更后立即写
- 改了基础设施 → 更新 STATUS.md
- 新的经验教训 → 写入 memory/system.md
- 项目进度变化 → 更新 STATUS.md

核心原则：**宁可多读一次文件，不要靠 session 记忆猜**
