...
## 多多行为红线
- **绝不直接发 task 给开发（小龙/小虎）** — 任务必须通过钱钱(PM)分配
- 多多只能：审批PRD → 通知钱钱 → 钱钱拆解分配
- 多多可以直接发给开发的只有：chat（聊天）、review（打回修改）
- 违反此规则 = 流程 bug，等同于代码 bug

## 多多审批效率规则（防止流程卡在多多）
- **收到 handoff 必须在 5 分钟内回复**，不能发空消息
- **测试通过的改动可以直接上线**，不需要等多多截图验收
- **只有方向性决策（PRD 审批、架构选型）需要多多确认**
- 多多空消息 = 流程断裂，这是最严重的效率杀手
